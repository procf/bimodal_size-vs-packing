# Analysis DPD Bimodal Colloid Sims (HOOMD-blue v.4.2.1-mod)
...
2. Update the filepaths sourced to in `package-install`, `compile-module`, and `analyze` to match your installation of HOOMD-blue.
...
- View the `sim-analysis-bi-colloids.py` `sim-networkCSV-bi-colloids.py` and `sim-voronoi-bi-colloids.py` files to select which analyses to run (comment out any unnecessary analyses in the "RUN CHECKS ON A SIMULATION" section).
- View the `analyze` file and select which python analysis scripts to run (comment out any unnecessary scripts)

6. Check outputs once the `data` folder has been generated and all scripts are completed without errors (no content in the error file and all desired analysis stated as completed in the out file)
